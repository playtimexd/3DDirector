...
it("uses a full-width director desk frame instead of floating card columns", () => {
  ...
  expect(screen.getByLabelText("Scene")).toHaveClass("left-sidebar");
  expect(screen.getByLabelText("3D Viewport")).toHaveClass("viewport-column");
  expect(screen.getByLabelText("Properties")).toHaveClass("right-sidebar");
});
...
it("collapses both side panels from the fullscreen toolbar action", async () => {
  ...
  expect(screen.getByLabelText("Scene")).toHaveAttribute("aria-hidden", "true");
  expect(screen.getByLabelText("Properties")).toHaveAttribute("aria-hidden", "true");
});
...
it("switches from director mode to camera mode", async () => {
  ...
  const directorButton = screen.getByRole("button", { name: "Director View" });
  const cameraButton = screen.getByRole("button", { name: "Camera View" });
