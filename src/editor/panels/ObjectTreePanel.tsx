...
export function ObjectTreePanel() {
  ...
  return (
    <section className="panel-card object-tree-panel">
      <h2 className="visually-hidden">Scene Objects</h2>
      <label className="object-search-field">
        <Search aria-hidden="true" size={16} strokeWidth={1.8} />
        <input
          className="ui-field"
          aria-label="Search scene contents"
          value={query}
      ...
      {hasEmptySearchResult ? (
        <div className="object-search-empty-state" role="status" aria-label="No results found">
        ...
        <div className="object-tree-groups" role="tree" aria-label="Scene object list">
          {filteredGroups.map((group) => (
            <section key={group.key} className="object-tree-group" role="group" aria-label={`${group.title} group`}>
              <h3>{group.title}</h3>
                  ...
                  return (
                    <li key={item.id} className="object-list-item">
                      ...
                      {item.crowdId && expanded && item.previewChildren?.length ? (
                        <ul className="object-crowd-preview-list" aria-label={`${item.name} member preview`}>
