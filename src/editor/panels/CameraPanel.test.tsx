import { fireEvent, render, screen, within } from "@testing-library/react";
import userEvent from "@testing-library/user-event";
import { afterEach, beforeEach, vi } from "vitest";
import { clearViewportCaptureHandler, setViewportCaptureHandler } from "../io/captureBridge";
import { createInitialDirectorState, useDirectorStore } from "../store/directorStore";
import { CameraPanel } from "./CameraPanel";

function seedCameraCapture() {
  useDirectorStore.setState((state) => ({
    ...state,
    project: {
      ...state.project,
      cameras: state.project.cameras.map((camera) =>
        camera.id === "cam_1"
          ? {
              ...camera,
              lastCaptureUrl: "data:image/png;base64,camera-preview",
              captures: [
                {
                  id: "cam_1-capture-01",
                  index: 1,
                  name: "Camera01-Capture01",
                  dataUrl: "data:image/png;base64,camera-preview",
                },
              ],
            }
          : camera
      ),
    },
  }));
}

function seedGroupedCameraCaptures() {
  const baseState = useDirectorStore.getState();
  const firstCamera = baseState.project.cameras[0];
  expect(firstCamera).toBeTruthy();

  useDirectorStore.setState({
    ...baseState,
    project: {
      ...baseState.project,
      cameras: [
        {
          ...firstCamera!,
          captures: [
            {
              id: "cam_1-capture-01",
              index: 1,
              name: "Camera01-Capture01",
              dataUrl: "data:image/png;base64,camera-1-a",
            },
            {
              id: "cam_1-capture-02",
              index: 2,
              name: "Camera01-Capture02",
              dataUrl: "data:image/png;base64,camera-1-b",
            },
          ],
          lastCaptureUrl: "data:image/png;base64,camera-1-b",
        },
        {
          ...firstCamera!,
          id: "cam_2",
          name: "Camera02",
          captures: [
            {
              id: "cam_2-capture-01",
              index: 1,
              name: "Camera02-Capture01",
              dataUrl: "data:image/png;base64,camera-2-a",
            },
          ],
          lastCaptureUrl: "data:image/png;base64,camera-2-a",
        },
      ],
    },
  });
}

beforeEach(() => {
  useDirectorStore.setState({
    ...useDirectorStore.getState(),
    ...createInitialDirectorState(),
    selectedObjectId: "cam_object_1",
  });
});

afterEach(() => {
  clearViewportCaptureHandler();
  vi.restoreAllMocks();
});

it("renders the approved camera panel fields", () => {
  render(<CameraPanel />);

  expect(screen.getByText("Camera")).toBeInTheDocument();
  expect(screen.getByLabelText("Camera name")).toBeInTheDocument();
  expect(screen.getByLabelText("Switch camera")).toBeInTheDocument();
  expect(screen.getByLabelText("Camera position X")).toBeInTheDocument();
  expect(screen.getByLabelText("Look-at target mode")).toBeInTheDocument();
  expect(screen.getByLabelText("Look-at coordinate X")).toBeInTheDocument();
  expect(screen.getByLabelText("Camera FOV")).toBeInTheDocument();
});

it("uses the provided right inspector layout for camera properties", () => {
  const { container } = render(<CameraPanel />);

  expect(screen.getByLabelText("Camera properties panel")).toHaveClass("right-inspector");
  expect(container.querySelector(".right-inspector-tabs")).toBeInTheDocument();
  expect(container.querySelector(".right-inspector-content")).toBeInTheDocument();
  expect(screen.queryByLabelText("Camera preview card")).not.toBeInTheDocument();
  expect(screen.queryByText("FOV 50°")).not.toBeInTheDocument();
  expect(container.querySelector("select.inspector-select-input")).not.toBeInTheDocument();
  expect(screen.getByLabelText("Switch camera")).toHaveClass("inspector-dropdown-trigger");
  expect(screen.getByLabelText("Look-at target mode")).toHaveClass("inspector-dropdown-trigger");

  const positionY = screen.getByLabelText("Camera position Y").closest(".inspector-axis-input");
  const fovField = screen.getByLabelText("Camera FOV").closest(".inspector-range-row");

  expect(positionY).toBeInTheDocument();
  expect(within(positionY as HTMLElement).getByText("Y")).toHaveClass("inspector-axis-prefix");
  expect(fovField).toBeInTheDocument();
});

it("keeps camera panel tab labels in fixed slots while switching tabs", async () => {
  const user = userEvent.setup();
  const { container } = render(<CameraPanel />);
  const tabList = container.querySelector(".right-inspector-tabs");
  const propertyTab = screen.getByRole("button", { name: "Properties" });
  const capturesTab = screen.getByRole("button", { name: "Camera captures" });

  expect(tabList).toHaveClass("right-inspector-tabs");
  expect(propertyTab).toHaveClass("right-inspector-tab-button");
  expect(capturesTab).toHaveClass("right-inspector-tab-button");
  expect(propertyTab).toHaveAttribute("aria-pressed", "true");

  await user.click(capturesTab);

  expect(propertyTab).toHaveClass("right-inspector-tab-button");
  expect(capturesTab).toHaveClass("right-inspector-tab-button");
  expect(capturesTab).toHaveAttribute("aria-pressed", "true");
});

it("updates the selected camera name and fov", async () => {
  const user = userEvent.setup();
  render(<CameraPanel />);

  await user.clear(screen.getByLabelText("Camera name"));
  await user.type(screen.getByLabelText("Camera name"), "Close-up camera");
  await user.clear(screen.getByLabelText("Camera FOV"));
  await user.type(screen.getByLabelText("Camera FOV"), "65");

  const camera = useDirectorStore.getState().project.cameras[0];
  expect(camera.name).toBe("Close-up camera");
  expect(camera.fov).toBe(65);
});

it("uses the custom dropdown menu to switch camera shots", async () => {
  const user = userEvent.setup();
  useDirectorStore.getState().addCameraShot();

  render(<CameraPanel />);

  await user.click(screen.getByLabelText("Switch camera"));

  const menu = screen.getByRole("listbox", { name: "Switch camera" });

  expect(menu).toHaveClass("inspector-dropdown-menu");

  await user.click(within(menu).getByRole("option", { name: "Camera01" }));

  expect(useDirectorStore.getState().project.activeCameraId).toBe("cam_1");
  expect(screen.queryByRole("listbox", { name: "Switch camera" })).not.toBeInTheDocument();
});

it("renders target mode as the custom dropdown menu", async () => {
  const user = userEvent.setup();

  render(<CameraPanel />);

  await user.click(screen.getByLabelText("Look-at target mode"));

  const menu = screen.getByRole("listbox", { name: "Look-at target mode" });
  const manualOption = within(menu).getByRole("option", { name: "Manual coordinates" });

  expect(menu).toHaveClass("inspector-dropdown-menu");
  expect(manualOption).toHaveClass("is-selected");
  expect(manualOption).toHaveAttribute("aria-selected", "true");
});

it("lists visible viewport models as camera focus targets and centers on the selected model", async () => {
  const user = userEvent.setup();
  useDirectorStore.getState().addPresetCharacter("female");
  useDirectorStore.getState().addGeometryPrimitive("box");

  render(<CameraPanel />);

  await user.click(screen.getByLabelText("Look-at target mode"));

  const menu = screen.getByRole("listbox", { name: "Look-at target mode" });
  const modelNames = within(menu).getAllByRole("option").map((option) => option.textContent);

  expect(modelNames).toEqual(["Manual coordinates", "Character01", "Character02", "Cube"]);
  expect(within(menu).queryByRole("option", { name: "Camera01" })).not.toBeInTheDocument();

  await user.click(within(menu).getByRole("option", { name: "Character02" }));

  const state = useDirectorStore.getState();
  const focusedObject = state.project.objects.find((item) => item.name === "Character02");
  const camera = state.project.cameras.find((item) => item.id === state.project.activeCameraId);

  expect(focusedObject).toBeTruthy();
  expect(camera?.targetMode).toBe("object");
  expect(camera?.targetObjectId).toBe(focusedObject?.id);
  expect(camera?.target).toEqual([-1.25, 0.99, 0]);
  expect(screen.getByLabelText("Look-at target mode")).toHaveTextContent("Character02");
  expect(screen.getByLabelText("Look-at coordinate X")).toHaveValue(-1.25);
  expect(screen.getByLabelText("Look-at coordinate Y")).toHaveValue(0.99);
  expect(screen.getByLabelText("Look-at coordinate Z")).toHaveValue(0);
});

it("updates camera position and target coordinates across all axes", async () => {
  const user = userEvent.setup();
  render(<CameraPanel />);

  await user.clear(screen.getByLabelText("Camera position Y"));
  await user.type(screen.getByLabelText("Camera position Y"), "3.4");
  await user.clear(screen.getByLabelText("Camera position Z"));
  await user.type(screen.getByLabelText("Camera position Z"), "7.5");
  await user.clear(screen.getByLabelText("Look-at coordinate Y"));
  await user.type(screen.getByLabelText("Look-at coordinate Y"), "1.8");
  await user.clear(screen.getByLabelText("Look-at coordinate Z"));
  await user.type(screen.getByLabelText("Look-at coordinate Z"), "2");

  const camera = useDirectorStore.getState().project.cameras[0];
  expect(camera.transform.position).toEqual([0, 3.4, 7.5]);
  expect(camera.target).toEqual([0, 1.8, 2]);
});

it("captures the current camera preview from the properties tab and shows it in the screenshots overview", async () => {
  const user = userEvent.setup();
  setViewportCaptureHandler(async () => [
    {
      label: "Current camera",
      dataUrl: "data:image/png;base64,camera-preview",
      meta: {
        mode: "camera",
        cameraId: "cam_1",
        fov: 50,
        position: [0, 2.2, 9],
        target: [0, 1.2, 0],
      },
    },
  ]);

  render(<CameraPanel />);

  await user.click(screen.getByRole("button", { name: "Capture current camera" }));
  await user.click(screen.getByRole("button", { name: "Camera captures" }));

  expect(useDirectorStore.getState().project.cameras[0]?.lastCaptureUrl).toBe("data:image/png;base64,camera-preview");
  expect(useDirectorStore.getState().project.cameras[0]?.captures).toEqual([
    {
      id: "cam_1-capture-01",
      index: 1,
      name: "Camera01-Capture01",
      dataUrl: "data:image/png;base64,camera-preview",
    },
  ]);
  expect(await screen.findByAltText("Camera01-Capture01 thumbnail")).toBeInTheDocument();
  expect(screen.getByRole("heading", { name: "Camera01 captures" })).toBeInTheDocument();
  expect(screen.getByText("Camera01-Capture01")).toBeInTheDocument();
});

it("keeps the camera capture section visible at the bottom of the properties tab", () => {
  render(<CameraPanel />);

  expect(screen.getByRole("heading", { name: "Camera capture" })).toBeInTheDocument();
  expect(screen.getByRole("button", { name: "Capture current camera" })).toHaveClass("camera-capture-current-button");
  expect(screen.getByTestId("camera-current-capture-icon")).toBeInTheDocument();
});

it("renders the thumbnail actions in a bottom bar and opens the project-style viewer toolbar", async () => {
  const user = userEvent.setup();
  seedCameraCapture();

  render(<CameraPanel />);

  expect(screen.getByRole("group", { name: "Camera01-Capture01 thumbnail actions" })).toHaveClass("camera-capture-actions");

  await user.click(screen.getByLabelText("View capture Camera01-Capture01"));

  const viewer = screen.getByRole("dialog", { name: "Camera capture viewer" });
  const toolbar = within(viewer).getByRole("toolbar", { name: "Camera capture viewer toolbar" });

  expect(viewer).toBeInTheDocument();
  expect(screen.getByAltText("Camera01-Capture01 enlarged view")).toBeInTheDocument();
  expect(within(toolbar).getByRole("button", { name: "Zoom in" })).toBeInTheDocument();
  expect(within(toolbar).getByRole("button", { name: "Zoom out" })).toBeInTheDocument();
  expect(within(toolbar).getByRole("button", { name: "Download image" })).toBeInTheDocument();
  expect(within(toolbar).getByRole("button", { name: "Close camera capture viewer" })).toBeInTheDocument();
});

it("sends a single camera capture to the host canvas when the thumbnail action is clicked", async () => {
  const user = userEvent.setup();
  const postMessage = vi.spyOn(window.parent, "postMessage").mockImplementation(() => undefined);
  seedCameraCapture();

  render(<CameraPanel />);

  await user.click(screen.getByRole("button", { name: "Send to canvas Camera01-Capture01" }));

  expect(postMessage).toHaveBeenCalledWith(
    {
      type: "storyai:director-desk-captures-sent",
      payload: {
        captures: [
          {
            dataUrl: "data:image/png;base64,camera-preview",
            fileName: "Camera01-Capture01.png",
          },
        ],
      },
    },
    window.location.origin
  );
});

it("shows all camera screenshots grouped by camera in the screenshots tab", async () => {
  const user = userEvent.setup();
  seedGroupedCameraCaptures();

  render(<CameraPanel />);

  await user.click(screen.getByRole("button", { name: "Camera captures" }));

  const firstGroup = screen.getByRole("region", { name: "Camera01 captures" });
  const secondGroup = screen.getByRole("region", { name: "Camera02 captures" });

  expect(screen.queryByRole("heading", { name: "Camera capture" })).not.toBeInTheDocument();
  expect(within(firstGroup).getByRole("heading", { name: "Camera01 captures" })).toBeInTheDocument();
  expect(within(firstGroup).getByText("Camera01-Capture01")).toBeInTheDocument();
  expect(within(firstGroup).getByText("Camera01-Capture02")).toBeInTheDocument();
  expect(within(secondGroup).getByRole("heading", { name: "Camera02 captures" })).toBeInTheDocument();
  expect(within(secondGroup).getByText("Camera02-Capture01")).toBeInTheDocument();
  expect(screen.getByRole("button", { name: "Clear all" })).toHaveClass("camera-capture-clear-all");
  expect(screen.getByRole("button", { name: "Send to canvas" })).toHaveClass(
    "camera-capture-send-all",
    "viewport-toolbar-crowd-confirm"
  );
  expect(screen.getByRole("button", { name: "Send to canvas" })).not.toHaveClass("is-hover-state");
  expect(screen.getByTestId("camera-capture-clear-icon")).toBeInTheDocument();
  expect(screen.getByTestId("camera-capture-send-icon")).toBeInTheDocument();

  const panel = screen.getByLabelText("Camera properties panel");
  const content = panel.querySelector(".right-inspector-content");
  const footer = panel.querySelector(".camera-capture-overview-footer");

  expect(footer).toBeInTheDocument();
  expect(content).toBeInTheDocument();
  expect(content).not.toContainElement(footer as HTMLElement);
});

it("sends all visible camera screenshots to the host canvas from the overview footer", async () => {
  const user = userEvent.setup();
  const postMessage = vi.spyOn(window.parent, "postMessage").mockImplementation(() => undefined);
  seedGroupedCameraCaptures();

  render(<CameraPanel />);

  await user.click(screen.getByRole("button", { name: "Camera captures" }));
  await user.click(screen.getByRole("button", { name: "Send to canvas" }));

  expect(postMessage).toHaveBeenCalledWith(
    {
      type: "storyai:director-desk-captures-sent",
      payload: {
        captures: [
          {
            dataUrl: "data:image/png;base64,camera-1-a",
            fileName: "Camera01-Capture01.png",
          },
          {
            dataUrl: "data:image/png;base64,camera-1-b",
            fileName: "Camera01-Capture02.png",
          },
          {
            dataUrl: "data:image/png;base64,camera-2-a",
            fileName: "Camera02-Capture01.png",
          },
        ],
      },
    },
    window.location.origin
  );
});

it("clears every camera screenshot from the screenshots tab and shows the empty state", async () => {
  const user = userEvent.setup();
  seedGroupedCameraCaptures();

  render(<CameraPanel />);

  await user.click(screen.getByRole("button", { name: "Camera captures" }));
  await user.click(screen.getByRole("button", { name: "Clear all" }));

  expect(screen.queryByText("Camera01-Capture01")).not.toBeInTheDocument();
  const emptyState = screen.getByRole("status", { name: "No camera captures yet" });
  expect(emptyState).toHaveClass("camera-capture-empty", "object-search-empty-state");
  expect(screen.getByTestId("camera-capture-empty-icon")).toHaveClass("object-search-empty-icon");
  expect(screen.getByTestId("camera-capture-empty-icon").querySelector(".lucide-images")).toBeInTheDocument();
  expect(screen.getByTestId("camera-capture-empty-icon").querySelector(".lucide-search")).not.toBeInTheDocument();
  expect(screen.getByText("No camera captures yet")).toBeInTheDocument();
  expect(useDirectorStore.getState().project.cameras.map((camera) => camera.captures ?? [])).toEqual([[], []]);
  expect(useDirectorStore.getState().project.cameras.map((camera) => camera.lastCaptureUrl ?? null)).toEqual([
    null,
    null,
  ]);
});

it("closes the capture viewer when clicking outside the image", async () => {
  const user = userEvent.setup();
  seedCameraCapture();

  const { container } = render(<CameraPanel />);

  await user.click(screen.getByLabelText("View capture Camera01-Capture01"));

  const previewImage = screen.getByAltText("Camera01-Capture01 enlarged view");
  const viewerStage = container.querySelector(".camera-capture-viewer-stage");

  expect(viewerStage).toBeInTheDocument();

  await user.click(previewImage);
  expect(screen.getByRole("dialog", { name: "Camera capture viewer" })).toBeInTheDocument();

  await user.click(viewerStage as HTMLElement);
  expect(screen.queryByRole("dialog", { name: "Camera capture viewer" })).not.toBeInTheDocument();
});

it("zooms the capture preview through the viewer toolbar controls with the canvas image preview step", async () => {
  const user = userEvent.setup();
  seedCameraCapture();

  render(<CameraPanel />);

  await user.click(screen.getByLabelText("View capture Camera01-Capture01"));
  await user.click(screen.getByRole("button", { name: "Zoom in" }));

  expect(screen.getByAltText("Camera01-Capture01 enlarged view")).toHaveStyle({
    transform: "translate(0px, 0px) scale(1.25)",
  });
});

it("supports wheel zooming and dragging like the canvas image preview", async () => {
  const user = userEvent.setup();
  seedCameraCapture();

  render(<CameraPanel />);

  await user.click(screen.getByLabelText("View capture Camera01-Capture01"));

  const previewImage = screen.getByAltText("Camera01-Capture01 enlarged view");

  fireEvent.wheel(previewImage, { deltaY: -100 });
  expect(previewImage).toHaveStyle({ transform: "translate(0px, 0px) scale(1.25)" });

  fireEvent.mouseDown(previewImage, { clientX: 100, clientY: 100 });
  fireEvent.mouseMove(window, { clientX: 124, clientY: 132 });
  fireEvent.mouseUp(window);

  expect(previewImage).toHaveStyle({ transform: "translate(24px, 32px) scale(1.25)" });
});

it("deletes a camera capture from the screenshot grid", async () => {
  const user = userEvent.setup();
  seedCameraCapture();

  render(<CameraPanel />);

  await user.click(screen.getByLabelText("Delete capture Camera01-Capture01"));

  expect(screen.queryByText("Camera01-Capture01")).not.toBeInTheDocument();
  expect(useDirectorStore.getState().project.cameras[0]?.captures).toEqual([]);
  expect(useDirectorStore.getState().project.cameras[0]?.lastCaptureUrl).toBeNull();
});
