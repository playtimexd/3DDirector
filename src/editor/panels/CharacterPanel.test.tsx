...
it("renders the approved role property order", () => {
  ...
  expect(screen.getByLabelText("Character position X")).toBeInTheDocument();
  expect(screen.getByLabelText("Character rotation X")).toBeInTheDocument();
  expect(screen.getByLabelText("Character scale X")).toBeInTheDocument();
  expect(screen.getByLabelText("Character uniform scale")).toBeInTheDocument();
  expect(screen.getByLabelText("Character color")).toBeInTheDocument();
});
...
it("keeps role axis labels exactly 10px above their coordinate rows", () => {
  ...
  ["Position", "Rotation", "Scale"].forEach((label) => {
...
it("uses the provided right inspector layout for role properties", () => {
  ...
  expect(screen.getByLabelText("Character properties panel")).toHaveClass("right-inspector", "character-inspector");
  ...
  const positionX = screen.getByLabelText("Character position X").closest(".inspector-axis-input");
  const colorRow = screen.getByLabelText("Character color HEX").closest(".inspector-color-row");
  ...
  expect(screen.getByLabelText("Character color")).toHaveClass("inspector-color-swatch");
});
...
it("marks the pose adjustment section for the compact character inspector layout", async () => {
  ...
  await user.click(screen.getByRole("button", { name: "Pose" }));

  expect(screen.getByText("Pose presets").closest(".inspector-section")).toHaveClass("pose-preset-section");
  expect(screen.getByText("Pose adjustment").closest(".inspector-section")).toHaveClass("pose-adjust-section");
});

it("adjusts axis values by dragging the gray XYZ prefix handles", () => {
  render(<CharacterPanel />);

  const dragHandle = screen.getByRole("button", { name: "Character position X drag to adjust" });
  ...
  expect(screen.getByLabelText("Character position X")).toHaveValue(2);
});
...
it("updates the selected role name and uniform scale", async () => {
  ...
  await user.type(screen.getByLabelText("Character name"), "Lead");
  await user.clear(screen.getByLabelText("Character uniform scale"));
  await user.type(screen.getByLabelText("Character uniform scale"), "1.2");
...
it("updates role rotation, per-axis scale, and hex color fields", async () => {
  ...
  await user.clear(screen.getByLabelText("Character rotation Y"));
  await user.type(screen.getByLabelText("Character rotation Y"), "15");
  await user.clear(screen.getByLabelText("Character scale Z"));
  await user.type(screen.getByLabelText("Character scale Z"), "1.4");
  await user.clear(screen.getByLabelText("Character color HEX"));
  await user.type(screen.getByLabelText("Character color HEX"), "#123456");
