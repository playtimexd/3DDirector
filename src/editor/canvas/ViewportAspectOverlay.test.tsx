...
describe("ViewportAspectOverlay", () => {
  ...
  it.each([
    ...
    const frame = screen.getByLabelText("Viewport frame");
  ...
  it("measures the frame when users switch from auto to a concrete aspect ratio", () => {
    ...
    expect(screen.queryByLabelText("Viewport frame")).not.toBeInTheDocument();
  ...
  it("shows the guide toggle button only when a concrete aspect ratio is active", () => {
    ...
    expect(screen.queryByRole("button", { name: "Show rule-of-thirds grid" })).not.toBeInTheDocument();

    rerender(<ViewportAspectOverlay ratio="16:9" showRuleOfThirds={false} onToggleRuleOfThirds={() => undefined} />);

    expect(screen.getByRole("button", { name: "Show rule-of-thirds grid" })).toBeInTheDocument();
  });
  ...
  it("renders the rule-of-thirds guides when enabled", () => {
    ...
    expect(screen.getByRole("button", { name: "Hide rule-of-thirds grid" })).toHaveAttribute("aria-pressed", "true");
