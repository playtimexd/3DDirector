...
it("renders a live R3F viewport and director scene controls", () => {
  ...
  expect(screen.getByLabelText("Scene scale")).toBeInTheDocument();
  expect(screen.getByText("Panorama background")).toBeInTheDocument();
  expect(screen.getByText("Panorama sphere")).toBeInTheDocument();
...
it("does not render a full-viewport transform drag layer over the 3D viewport", () => {
  ...
  expect(screen.queryByRole("application", { name: "3D viewport move drag layer" })).not.toBeInTheDocument();
  expect(screen.queryByRole("application", { name: "3D viewport rotate drag layer" })).not.toBeInTheDocument();
  expect(screen.queryByRole("application", { name: "3D viewport scale drag layer" })).not.toBeInTheDocument();
});
...
it("opens the scene inspector when users click empty 3D viewport space", () => {
  ...
  expect(screen.getByLabelText("3D scene properties panel")).toBeInTheDocument();
  expect(screen.getByLabelText("Scene scale")).toBeInTheDocument();
});
...
it("renders the viewport aspect ratio overlay when a non-auto frame is selected", () => {
  ...
  expect(screen.getByLabelText("Viewport frame")).toBeInTheDocument();
  expect(screen.getAllByLabelText("Viewport frame mask")).toHaveLength(4);
  expect(screen.getByLabelText("Viewport frame")).toHaveAttribute("data-aspect-ratio", "9:16");
});
...
it("toggles the viewport rule-of-thirds guide from the aspect frame button", () => {
  ...
  const guideToggle = screen.getByRole("button", { name: "Show rule-of-thirds grid" });
  fireEvent.click(guideToggle);

  expect(useDirectorStore.getState().viewportRuleOfThirdsEnabled).toBe(true);
  expect(screen.getByLabelText("Rule-of-thirds grid")).toBeInTheDocument();
});
...
it("renders the native 3D viewport gizmo in an overlay canvas above the aspect mask", () => {
  ...
  expect(screen.getByLabelText("3D viewport gizmo")).toContainElement(screen.getByTestId("native-gizmo-helper"));
  ...
  expect(screen.queryByLabelText("3D viewport axis indicator")).not.toBeInTheDocument();
});

it("offsets the native viewport gizmo inward when overlay side panels are open", () => {
  render(<App />);

  const gizmo = screen.getByLabelText("3D viewport gizmo");
...
it("syncs native viewport gizmo axis clicks back to the main director view", () => {
  ...
  const xAxisHitTarget = screen.getByRole("button", { name: "Switch to +X view" });
...
it("draws visible character name labels into viewport screenshots", async () => {
  ...
  expect(fillText).toHaveBeenCalledWith("Character01", expect.any(Number), expect.any(Number));
});
