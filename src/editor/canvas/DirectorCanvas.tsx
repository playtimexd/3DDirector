...
const GIZMO_AXIS_HIT_TARGETS: Array<{
  ...
  { label: "Switch to +X view", className: "is-x-positive", direction: [1, 0, 0] },
  { label: "Switch to +Y view", className: "is-y-positive", direction: [0, 1, 0] },
  { label: "Switch to +Z view", className: "is-z-positive", direction: [0, 0, 1] },
  { label: "Switch to -X view", className: "is-x-negative", direction: [-1, 0, 0] },
  { label: "Switch to -Y view", className: "is-y-negative", direction: [0, -1, 0] },
  { label: "Switch to -Z view", className: "is-z-negative", direction: [0, 0, -1] },
];
...
function CanvasCaptureBridge({
  ...
  useEffect(() => {
    ...
    const capture = async ({
      ...
      if (preset === "current") {
        return [snapshot(source === "camera-panel" ? "Current camera" : "Current view")];
      }

      const count = preset === "four" ? 4 : 12;
      const labelPrefix = preset === "four" ? "Angle" : "Angle";
...
function ViewportGizmoOverlay({
  ...
  return (
    <div className="viewport-gizmo-overlay" aria-label="3D viewport gizmo" style={{ right: `${rightOffset}px` }}>
      <Canvas
      ...
      <div className="viewport-gizmo-hit-layer" aria-label="3D viewport axis buttons">
