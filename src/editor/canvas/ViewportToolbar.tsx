...
export function ViewportToolbar({
  ...
  const actions: ToolbarAction[] = [
    { label: "Move", icon: Move3D, mode: "translate", onClick: () => selectTransformMode("translate") },
    { label: "Rotate", icon: Rotate3D, mode: "rotate", onClick: () => selectTransformMode("rotate") },
    { label: "Scale", icon: Scale3D, mode: "scale", onClick: () => selectTransformMode("scale") },
    { label: "Import panorama", icon: ImagePlus, onClick: () => panoramaInputRef.current?.click() },
    {
    ...
    { label: "Model Library", icon: Boxes, onClick: toggleModelLibrary },
    { label: "Add camera", icon: Video, onClick: addCameraFromViewport },
    { label: "Select aspect ratio", icon: Ratio, onClick: toggleAspectRatioPanel },
    { label: "Capture current view", icon: Camera, onClick: () => void handleCapture("current") },
    { label: "Four-angle capture", icon: Grid2X2, onClick: () => void handleCapture("four") },
    { label: "Twelve-angle capture", icon: Grid3X3, onClick: () => void handleCapture("twelve") },
    { label: "Fullscreen", icon: Expand, onClick: toggleViewportPanelsCollapsed },
  ];
  ...
  return (
    <>
      <div className="viewport-toolbar" role="group" aria-label="3D viewport tools" ref={setToolbarElement}>
        ...
        <div className="viewport-toolbar-menu-wrap">
          ...
          >
            ...
            <span className="viewport-toolbar-label">Add character</span>
          </button>
        </div>
        {actions.slice(3).map((action) => {
          if (action.label !== "Model Library") {
      ...
      {crowdPanelOpen ? (
        <div
          ...
          role="dialog"
          aria-label="Add crowd array"
          style={crowdPanelStyle}
        >
          <div className="viewport-toolbar-crowd-panel-header">
            <h2 className="viewport-toolbar-crowd-panel-title">Add crowd array</h2>
            <span className="viewport-toolbar-crowd-panel-count">{crowdTotalCount} total</span>
          </div>
      ...
      {modelLibraryOpen ? (
        ...
        >
          ...
          <div className="model-library-tabs" role="tablist" aria-label="Model categories">
          ...
          {activeModelLibraryCategoryId === "my-models" && activeModelLibraryItems.length === 0 ? (
            <div className="model-library-empty-state object-search-empty-state" role="status" aria-label="No models yet">
            ...
            <div className="model-library-grid" role="list" aria-label="Model list">
              {activeModelLibraryItems.map((item) => (
                activeModelLibraryCategoryId === "my-models" ? (
                  <div key={item.id} className="model-library-card-wrap">
                    <button
                      aria-label={`Add model ${item.name}`}
              ...
              {activeModelLibraryCategoryId === "my-models" ? (
                ...
                >
                  ...
                  <span className="model-library-name">Import local</span>
                </button>
              ) : null}
            </div>
          )}
      ...
      {aspectRatioPanelOpen ? (
        ...
        >
          <h2 className="viewport-aspect-panel-title">Aspect Ratio</h2>
          <div className="viewport-aspect-panel-grid" role="group" aria-label="Aspect ratio options">
