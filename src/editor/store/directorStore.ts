...
function formatSceneItemName(prefix: "Character" | "Camera", index: number) {
...
export function createDefaultDirectorProject({
  ...
  const camera: DirectorCameraShot = {
    id: "cam_1",
    name: formatSceneItemName("Camera", 1),
  ...
  const role: DirectorObject = {
    id: "char_default_a",
    name: formatSceneItemName("Character", 1),
    kind: "character",
    visible: true,
    locked: false,
...
function getGeometryPrimitiveLabel(geometryType: GeometryPrimitiveType) {
  return GEOMETRY_PRIMITIVE_OPTIONS.find((option) => option.type === geometryType)?.label ?? "Geometry";
}
...
function buildPresetCharacterObject(
  ...
  return {
    id: objectId,
    name: formatSceneItemName("Character", characterIndex),
...
function formatCameraCaptureName(cameraName: string, captureIndex: number) {
  return `${cameraName}-Capture${String(captureIndex).padStart(2, "0")}`;
}
...
function createDisplayNameFromFileName(fileName: string) {
  return fileName.replace(/\.(fbx|obj|glb|gltf|jpe?g|png|webp)$/i, "");
}
  ...
  state.clipboard.forEach((entry) => {
    if (entry.object.kind === "camera" && entry.camera) {
      ...
      const nextCamera: DirectorCameraShot = {
        ...entry.camera,
        id: nextCameraId,
        name: formatSceneItemName("Camera", cameraIndex),
    ...
    const duplicatedObject: DirectorObject = {
      ...
      name:
        entry.object.kind === "character" && nextCharacterCount
          ? formatSceneItemName("Character", nextCharacterCount)
          : entry.object.name,
