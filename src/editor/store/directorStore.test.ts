...
it("seeds the demo with one mannequin role and one camera", () => {
  ...
  expect(defaultCharacter?.name).toBe("Character01");
  expect(defaultCameraObject?.name).toBe("Camera01");
  expect(state.project.cameras[0]?.name).toBe("Camera01");
...
it("adds preset characters with a requested body type", () => {
  ...
  expect(added?.name).toBe("Character02");
...
it("adds camera shots with two-digit camera names", () => {
  ...
  expect(state.project.cameras.map((camera) => camera.name)).toEqual(["Camera01", "Camera02"]);
...
it("places newly added preset characters far enough from the default role to avoid overlap", () => {
  ...
  const role02 = characters.find((item) => item.name === "Character02");
  const role03 = characters.find((item) => item.name === "Character03");
...
it("keeps object-focused cameras centered when the target model moves", () => {
  ...
  const targetObject = useDirectorStore.getState().project.objects.find((item) => item.name === "Cube");
...
it("auto-persists the latest director scene snapshot after scene changes", () => {
  ...
  expect(parsed.project?.objects?.some((item) => item.name === "Character02")).toBe(true);
});
...
it("undoes the latest scene mutation", () => {
  ...
  expect(useDirectorStore.getState().project.objects.some((item) => item.name === "Character02")).toBe(true);
