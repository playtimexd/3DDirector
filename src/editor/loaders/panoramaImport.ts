...
async function readImageSource(file: File): Promise<PanoramaImageSource & CanvasImageSource> {
  ...
  return await new Promise<HTMLImageElement>((resolve, reject) => {
    ...
    image.onerror = () => {
      URL.revokeObjectURL(probeUrl);
      reject(new Error("Could not read panorama dimensions, please choose another image"));
    };
...
async function buildAdaptedPanoramaAsset(file: File) {
  ...
  try {
    ...
    if (!context) {
      throw new Error("Cannot generate panorama in this environment, please try again later");
    }
...
export async function readPanoramaFile(file: File) {
  if (!PANORAMA_IMAGE_EXTENSION_RE.test(file.name)) {
    throw new Error("Panorama images support only JPG / PNG / WEBP");
  }
